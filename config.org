...
(use-package olivetti
  :ensure t
  :hook ((org-mode . my/adaptive-olivetti-setup)
         (org-agenda-mode . my/adaptive-olivetti-setup)
  ...
  (defun my/adaptive-olivetti-setup ()
    "安全版本的自适应布局"
    (interactive)
    (when (derived-mode-p 'org-mode 'org-agenda-mode)
      (let* ((screen-width (frame-width))
             (base-width (cond ((> screen-width 200) 130)
                               ((> screen-width 150) 110)
                               (t 100)))
             (margin-size (max 2 (/ (- screen-width base-width) 4)))
             (bg-color (face-attribute 'default :background)))
        ;; 设置 face 属性
        (set-face-attribute 'olivetti-default-face nil
                           :background bg-color
                           :foreground (face-attribute 'default :foreground))
        
        ;; 应用布局设置
        (setq olivetti-body-width base-width
              left-margin-width margin-size
